...
> What each zip holds is in [engine/crawl-archives/README.md](engine/crawl-archives/README.md),
> and the full data guide is [DATA_GUIDE.md](DATA_GUIDE.md).
...
### Restore the heavy data

Scraped PDFs, the raw API dump and every crawl run are too large to commit as
loose files. Each website's are committed as **one zip** in
`engine/crawl-archives/`, stored with [Git LFS](https://git-lfs.com). Install
Git LFS before you clone, then:

```powershell
git lfs install
git lfs pull
Expand-Archive engine/crawl-archives/cuet.zip -DestinationPath engine/corpus
Expand-Archive engine/crawl-archives/bubt.zip -DestinationPath engine/data/sites   # likewise for each site
```

What each zip holds, and how to add to one, is in
[engine/crawl-archives/README.md](engine/crawl-archives/README.md).

The CUET scraper specifications (`CUET_SCRAPER_SPEC.md`,
`CUET_SCRAPER_SPEC_PART2.md`) are not in the repository or the zips. They are
kept locally by the CUET scraper's owner and are gitignored.
...
└── engine/                   everything lives here
    ...
    │       └── _files/           PDFs, zipped in crawl-archives/cuet.zip
    │
    ├── crawl-archives/       ONE ZIP PER WEBSITE, Git LFS: CUET's PDFs and every crawl run
    │
